...
UINT				get_weird_color(t_itype coord, t_meta *meta)
{
	t_itype			z;
	t_itype			d;
	int				i;
	...
	{
		z.i = 2 * z.i * z.r + z.i + coord.i;
		z.r = d.r - d.i + z.r + coord.r;
		d.r = z.r * z.r;
		d.i = z.i * z.i;
		i++;
	}
...
void				*dis_weird_thread(void *data)
{
	t_data			d;
	int				i;
	int				lin;
	...
	while (i < max)
	{
		...
		while (i < lin)
		{
			put_pixel(i, get_weird_color(coord, d.meta), d.meta);
			coord.r += d.meta->upp;
			i += d.meta->inc;
		}
	}
...
int					set_weird_thread(t_meta *meta)
{
	...
	while (++i < THREAD_NUM)
	{
		...
		rc = pthread_create(&t[i], NULL, dis_weird_thread, &d[i]);
		if (rc)
			error_thread(meta);
	}
	i = -1;
