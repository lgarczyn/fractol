...
UINT				get_mandelbrot_color(t_itype coord, t_meta *meta)
{
	t_itype			z;
	t_itype			d;
	int				i;
...
void				*dis_mandelbrot_thread(void *data)
{
	t_data			d;
	int				i;
	int				lin;
	...
	while (i < max)
	{
		...
		while (i < lin)
		{
			put_pixel(i, get_mandelbrot_color(coord, d.meta), d.meta);
			coord.r += d.meta->upp;
			i += d.meta->inc;
		}
	}
...
int					set_mandelbrot_thread(t_meta *meta)
{
	...
	while (++i < THREAD_NUM)
	{
		...
		rc = pthread_create(&t[i], NULL, dis_mandelbrot_thread, &d[i]);
		if (rc)
			error_thread(meta);
	}
	i = -1;
