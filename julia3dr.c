...
UINT				get_julia3dr_color(t_itype coord, int j, t_data3d t)
{
	t_itype			z;
	t_itype			d;
	int				i;
	...
	t.param.r -= 0.01;
	if (j < 256 - 2)
		return (get_julia3dr_color(coord, j + 2, t));
	return (BASE_COLOR);
}

void				*dis_julia3dr_thread(void *data)
{
	t_data3d		d;
	int				i;
	int				lin;
	...
	while (i < max)
	{
		...
		while (i < lin)
		{
			put_pixel(i, get_julia3dr_color(coord, 0, d), d.meta);
			coord.r += d.meta->upp;
			i += d.meta->inc;
		}
	}
...
int					dis_julia3dr(t_meta *meta)
{
	...
	data.thn = 1;
	data.meta = meta;
	data.param = (t_itype){meta->julia_param.r, 0.6};
	dis_julia3dr_thread(&data);
...
int					set_julia3dr_thread(t_meta *meta)
{
	...
	while (++i < THREAD_NUM)
	{
		param = (t_itype){meta->julia_param.r, 0.6};
		d[i] = (t_data3d){i, THREAD_NUM, meta, param};
		rc = pthread_create(&t[i], NULL, dis_julia3dr_thread, &d[i]);
		if (rc)
			error_thread(meta);
	}
	i = -1;
